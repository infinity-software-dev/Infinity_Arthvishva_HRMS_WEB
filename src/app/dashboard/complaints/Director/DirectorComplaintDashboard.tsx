...
import { Search, ShieldAlert, Archive } from 'lucide-react';
import DirectorLiveComplaintsList from './DirectorLiveComplaintsList';
import DirectorHistoryComplaintsList from './DirectorHistoryComplaintsList';

export default function DirectorComplaintDashboard() {
    const [activeTab, setActiveTab] = useState<'Live' | 'History'>('Live');
    const [searchQuery, setSearchQuery] = useState('');

    // Only forward search query to child components if length > 1 character
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 border-b border-gray-100 dark:border-gray-800 pb-4">
                ...
                <div className="relative w-full sm:w-64">
                    ...
                    <input
                        ...
                        className="w-full pl-9 pr-4 py-2 border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-white/5 text-primary dark:text-white text-sm rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-green transition-colors"
                    />
                </div>
            </div>
            ...
            <div className="min-h-[400px]">
                {activeTab === 'Live' ? (
                    <div className="w-full pt-4">
                        <DirectorLiveComplaintsList searchQuery={activeSearchQuery} />
                    </div>
                ) : (
                    <div className="w-full pt-4">
                        <DirectorHistoryComplaintsList searchQuery={activeSearchQuery} />
                    </div>
                )}
            </div>
