...
import { Eye, CheckCircle2, XCircle, MinusCircle, Crown, Archive } from 'lucide-react';
import { useDirectorHistoryComplaints } from '@/hooks/complaint-hooks/useDirectorHistoryComplaints';
...
interface DirectorHistoryComplaintsListProps {
    searchQuery: string;
}

export default function DirectorHistoryComplaintsList({ searchQuery }: DirectorHistoryComplaintsListProps) {
    const { complaints, isLoading, refetch } = useDirectorHistoryComplaints(searchQuery);
    ...
    if (isLoading) {
        return (
            <div className="flex flex-col items-center justify-center h-64 w-full">
                ...
                <p className="text-gray-500 dark:text-gray-400 text-sm font-medium">Loading director complaint history...</p>
            </div>
        );
    }
        ...
        return (
            <div className="flex flex-col items-center justify-center h-64 w-full border-2 border-dashed border-gray-200 dark:border-gray-800 rounded-xl bg-gray-50/50 dark:bg-white/5">
                ...
                <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">
                    {searchQuery ? 'No resolved or closed tickets match your search query.' : 'There are currently no closed complaint records in the director archive.'}
                </p>
            </div>
        );
    }
                        ...
                        {complaints.map((complaint) => {
                            ...
                            return (
                                <tr key={complaint._id} className="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors group">
                                    ...
                                    {/* Priority */}
                                    <td className="p-4">
                                        <span className={`px-2.5 py-1 text-xs font-bold rounded-md border ${getPriorityStyles(complaint.priority)}`}>
                                            {complaint.priority}
                                        </span>
                                    </td>
