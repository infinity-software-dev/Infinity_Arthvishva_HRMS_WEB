...
export const portalAttendanceService = {
    ...
    async getSystemConfig(): Promise<SystemConfig> {
        try {
            ...
            const response = await apiClient.get(EMPLOYEE_API.SYSTEM_CONFIGS);
    ...
    async checkIn(payload: CheckInPayload): Promise<any> {
        try {
            const response = await apiClient.post(EMPLOYEE_API.CHECK_IN, payload);
    ...
    async getTodayStatus(): Promise<RawAttendanceDocument | null> {
        try {
            const response = await apiClient.get(EMPLOYEE_API.GET_ATTENDANCE_STATUS);
    ...
    async getReportingManagers(): Promise<any[]> {
        try {
            const response = await apiClient.get('/api/web/attendance/reporting-managers');
    ...
    async checkOut(payload: any): Promise<any> {
        try {
            const response = await apiClient.post('/api/web/attendance/check-out', payload);
            return response.data;
        } catch (error: any) {
    ...
    async getMonthlyAttendance(year: string, month: string): Promise<any> {
        try {
            const response = await apiClient.get(`${EMPLOYEE_API.GET_MONTHLY_ATTENDANCE}?year=${year}&month=${month}`);
    ...
    async requestCorrection(
        ...
        try {
            const response = await apiClient.patch(EMPLOYEE_API.REQUEST_CORRECTION(attendanceId), payload);
            return response.data;
        } catch (error: any) {
